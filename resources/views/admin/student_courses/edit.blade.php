...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action=" {{ route('student_course_update',$course->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_duration') ? 'is-invalid' : '' }}" type="number" name="course_duration" id="course_duration" value="{{ old('course_duration', $course->course_duration) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course_type') ? 'is-invalid' : '' }}" name="course_type_id" id="course_type_id" required>
                    @foreach($course_types as $id => $course_type)
                        <option value="{{ $id }}" {{ ($course->course_type ? $course->course_type->id : old

                            ('course_type_id')) == $id ? 'selected' : '' }}>{{ $course_type }}</option>
                    @endforeach
                </select>
