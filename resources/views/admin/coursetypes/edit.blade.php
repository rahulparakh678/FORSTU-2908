...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.coursetypes.update", [$coursetype->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="course_type_name">{{ trans('cruds.coursetype.fields.course_type_name') }}</label>
                <input class="form-control {{ $errors->has('course_type_name') ? 'is-invalid' : '' }}" type="text" name="course_type_name" id="course_type_name" value="{{ old('course_type_name', $coursetype->course_type_name) }}">
