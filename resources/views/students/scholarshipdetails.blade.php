...
{{--
@if(isset($doc_names))
    @if($doc_names->isNotEmpty())
    <div class="card">
        <div class="row">
            <div class="card-body">
                <h4>Missing Documents:</h4>
                <ul>
                    @foreach($doc_names as $document)
                    <li>{{ $document}}</li>
                    @endforeach
                </ul>
            </div>
                ...
                <div class="card-body">
                   ...
                   <i class="fa fa-graduation-cap"></i>
                    ...
                    <i class="far fa-calendar-check"></i>
                        ...
                        <center><a href="{{route('applynowsection',$scholarships->id)}}" target="__blank" class="btn btn-primary">Apply Now</a></center>
                        
                        @else
                       {{-- <a href="{{route('applicationwindow',$scholarships->id)}}" class="btn btn-success" >Apply Now</a>
                       
                        <a href="{{ route('applicationwindow',['id' => $scholarships->id]) }}" class="btn btn-success">Apply Now</a>
                        --}}
                        {{--
                        <form action="{{ route('scholarship.check-documents',$scholarships->id)}} " method="POST" enctype="multipart/form-data" id="application-form">
                                        @csrf
                                        <center>
                                            <button class="btn btn-primary  " type="submit" id="submit-application">Apply Now</button>
                                        </center> 
                                    </form>
                         --}}   
